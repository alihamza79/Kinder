...
const BlogSliderPostPage = (props) => {
                  ...
                  <Row>
                    <Col className="blog-details-text last:mb-0 mb-24">
                      <ul className="flex mb-8 xs:block">
                        ...
                        {
                          authorData && (
                            <li className="inline-block align-middle"><i className="feather-user text-fastblue mr-[10px]"></i>By <Link to={`/blogs/author/${data[0].author}`}>{authorData.filter(author => author.id === data[0].author)[0].name}</Link></li>
                          )
                        }
                      </ul>
                      <h5 className="font-serif font-medium text-darkgray mb-[4.5rem]">{data[0].title}</h5>
                      <div className="relative mb-[4.5rem]">
                        <div onClick={() => swiperRef.current.swiper.slidePrev()} className="btn-slider-next bg-[#fff] z-[11] text-[#000] w-[40px] h-[40px] left-[0px] leading-[40px] m-0 right-auto absolute top-1/2 flex item-center transition-default justify-center hover:bg-[#000] hover:text-[#fff]" >
                          <button className="text-xmd"><i className="feather-arrow-left"></i></button>
                        </div>

                        <div onClick={() => swiperRef.current.swiper.slideNext()} className="btn-slider-prev btn-slider-next bg-[#fff] z-[11] text-[#000] w-[40px] h-[40px] right-[0px] leading-[40px] m-0 left-auto absolute top-1/2 flex item-center transition-default justify-center hover:bg-[#000] hover:text-[#fff]" >
                          <button className="text-xmd"><i className="feather-arrow-right"></i></button>
                        </div>
                        <Swiper
                          className="white-move rounded-[6px] swiper-pagination-medium h-full"
                          ref={swiperRef}
                          spaceBetween={30}
                          slidesPerView={1}
                          loop={true}
                              ...
                              return (
                                <SwiperSlide key={i}>
                                  <div className="h-full shadow-lg bg-white ">
                                    <img src={item.img} alt="" height="" width="" />
                                  </div>
                                </SwiperSlide>
                              )
                            })
                      ...
                      <Blockquote
                        ...
                        author="JOHN WAYNE"
                      />
                      <img height="" width="" src="https://via.placeholder.com/780x500" alt="" className="w-full rounded-[6px] mb-16" />
...
export default BlogSliderPostPage