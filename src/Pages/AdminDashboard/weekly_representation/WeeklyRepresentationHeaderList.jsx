...
const WeeklyRepresentationHeaderList = () => {
  const [dataSource, setDataSource] = useState([]);
  const [loading, setLoading] = useState(false);
  const location = useLocation();

  useEffect(() => {
    const updateSuccess = sessionStorage.getItem("updateWeeklyRepresentationHeaderSuccess");
    const addSuccess = sessionStorage.getItem("addWeeklyRepresentationHeaderSuccess");
    if (updateSuccess) {
      toast.success("Document updated successfully!", { autoClose: 2000 });
      sessionStorage.removeItem("updateWeeklyRepresentationHeaderSuccess");
    }
    if (addSuccess) {
      toast.success("Document Added successfully!", { autoClose: 2000 });
      sessionStorage.removeItem("addWeeklyRepresentationHeaderSuccess");
    }
    fetchData();
  }, [location]);
          ...
          <div className="dropdown dropdown-action">
            ...
            <div className="dropdown-menu dropdown-menu-end">
              <Link
                ...
                to={`/weeklyrepresentationheader/editweeklyrepresentationheader/${record.id}`}
              >
                      ...
                      <div className="row align-items-center">
                        <div className="col">
                          <div className="doctor-table-blk">
                            <h3>Weekly Representation Headers</h3>
                          </div>
                        </div>
                      </div>
                    </div>
...
export default WeeklyRepresentationHeaderList;
